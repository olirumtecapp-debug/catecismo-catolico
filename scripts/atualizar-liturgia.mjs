...
async function yearIsValid(year){
  if(!(await yearLooksComplete(year))) return false;
  try {
...
async function main(){
  ...
  for(const year of targets){
    if(!force && await yearIsValid(year)){
      console.log(`SKIP ${year}: base completa e validada, mantendo arquivos existentes.`);
      continue;
    }
    if(!force && await yearLooksComplete(year)) {
      console.log(`REBUILD ${year}: existem 365 arquivos, mas a validação encontrou problemas.`);
    } else {
